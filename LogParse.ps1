    ...
    if ($matchCount -ge $minMatchCount) {
      ...
      $DiskList = foreach ($match in $matches) {
        [PSCustomObject]@{
          ...
          Size          = [int64]([int64]$match.Groups['Capacity'].Value / 2048)
        }
      }
      ...
      foreach ($item in $SortedDiskList) {
        $DiskMap[$item.ID.ToString()] = "$($item.VendorProduct) REV:$($item.Revision) (SN:$($item.SerialNumber)) (Capacity:$($item.Size) MB)"
      }
      ...
      $patternMatchOutPath = Join-Path $OutPutDir $patternMatchFileName
      ...
      if ( $drvFailedMatches) {
        ...
        if ( $evtList) {
          ...
          # 3. 儲存與輸出
          $reportOutput | Out-File -FilePath $finalOutputPath -Encoding UTF8
          # 將所有匹配的行內容寫入該個別檔案
